...
layui.define(['layer', 'laytpl', 'upload'], function (exports) {
    ...
    var defaultAvatar = 'images/default_headicon.png';
    ...
    //修改群名称
    LAYIM.prototype.setGroupName = function (groupId, name) {
        var local = layui.data('layim')[cache.mine.id] || {};
        var history = local.history;
        history["group"+groupId].name = name;
        ...
        $("img.layim-group"+groupId).next().children("span").html(name);
    };
    ...
    var listTpl = function (options) {
        ...
        options.item.mine = options.mine;

        return ['{{# var length = 0; layui.each(' + options.item + ', function(i, data){ length++; }}'
            , '<li {{# if(data.id!=(d.mine?d.mine.id:0)) {}}layim-event="chat"{{#}}} data-type="' + options.type + '" data-index="{{ ' + (options.index || 'i') + ' }}" ' + (options.type != 'group' ? 'data-list="{{data.id}}"' : 'data-owner="{{data.owner.uid}}"') + ' data-id="{{data.id}}" class="layim-' + (options.type === 'history' ? '{{i}}' : options.type + '{{data.id}}') + ' {{ data.status === "offline" ? "layim-list-gray" : "" }}"><img src="{{ data.avatar || (data.base?data.base.defaultAvatar:"images/default_headicon.png") }}"><span>{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p><span class="layim-msg-status">new</span></li>'
            , '{{# }); if(length === 0){ }}'
    ...
    //start 组织模板
    var orgTpl = function () {
        return ['<ul class="layui-unselect layim-tab-content layim-list-org">'
            , '<ul id="esOrgTree"></ul>'
            , '</ul>'].join('');
    };
    //end 组织模板
    ...
    var elemTpl = ['<div class="layui-layim-main">'
        ...
        , '</div>'
        , '<div class="layui-layim-tools">'
        , '<a class="layui-layim-tool-item" href="moment/index" target="_blank"><i class="ali-icon icon-cntencentwechatmoments"></i><span class="badge badge-pill badge-danger invisible" id="momentNum">0</span></a>'
        , '<a class="layui-layim-tool-item" href="mail/inBox" target="_blank"><i class="fa fa-envelope-o"></i></a>'
        , '</div>'
        // , '<ul class="layui-unselect layui-layim-tab{{# if(!d.base.isfriend || !d.base.isgroup){ }}'
        , '<ul class="layui-unselect layui-layim-tab{{# if(!d.base.isfriend || !d.base.isOrg || !d.base.isgroup){ }}'
        , ' layim-tab-two'
        , '{{# } }}">'
        , '<li class="layui-icon'
        , '{{# if(!d.base.isfriend){ }}'
        ...
        , ' layim-this'
        , '{{# } }}'
        , '" title="组织机构" layim-event="tab" lay-type="org"><i class="fa fa-home" aria-hidden="true"></i></li>'
        //end 增加组织
        , '<li class="layui-icon'
        , '{{# if(!d.base.isgroup){ }}'
        , ' layim-hide'
        , '{{# } else if(!d.base.isOrg && !d.base.isfriend) { }}'
        , ' layim-this'
        , '{{# } }}'
        ...
        , '<li>'
        , '<h5 layim-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"未命名分组"+index }}</span></h5>'
        ...
        , '</ul>'
        , '</li>'
        , '{{# }); if(d.friend.length === 0 && d.org.length === 0){ }}'
        , '<li><ul class="layui-layim-list layui-show"><li class="layim-null">暂无联系人</li></ul>'
        , '{{# } }}'
        , '</ul>'
        //start 组织
        , '<ul class="layui-unselect layim-tab-content {{# if(!d.base.isfriend&&d.base.isgroup){ }}layui-show{{# } }} layim-list-org">'
        , '{{# layui.each(d.org, function(index, item){ var spread = d.local["spread"+index]; }}'
        , '<li>'
        , '<h5 layim-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.name||"未命名分组"+index }}</span></h5>'
        , '<ul class="layui-layim-list {{# if(spread === "true"){ }}'
        , ' layui-show'
        , '{{# } }}">'
        , '{{# layui.each(item.children, function(index, item){ var spread = d.local["spread"+index]; }}'
        , '<li class="list-dept">'
        , '<h5 layim-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.name||"未命名分组"+index }}</span></h5>'
        , '<ul class="layui-layim-list layui-layim-list-second {{# if(spread === "true"){ }}'
        , ' layui-show'
        , '{{# } }}">'
        , '{{# layui.each(item.children, function(index, item){ var spread = d.local["spread"+index]; }}'
        , '<li class="list-dept">'
        ...
        , '</ul>'
        //end 组织
        , '<ul class="layui-unselect layim-tab-content {{# if(!d.base.isOrg && !d.base.isfriend && d.base.isgroup){ }}layui-show{{# } }}">'
        , '<li>'
    ...
    var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
        ...
        , '<a href="javascript:;" layim-event="{{ d.data.type==="group" ? \"groupMembers\" : \"userInfo\" }}"><img class="layim-{{ d.data.type }}{{ d.data.id }}" src="{{ (d.data.mine? d.data.targetAvatar : d.data.avatar) || d.base.defaultAvatar }}"><span class="layim-chat-username"><span>{{ (d.data.mine? d.data.targetName: d.data.name ) || d.data.name ||"佚名" }}</span> {{d.data.temporary ? "<cite>临时会话</cite>" : ""}} {{# if(d.data.type==="group"){ }} <em class="layim-chat-members"></em><i class="layui-icon">&#xe61a;</i> {{# } }}</span></a>'
        ...
        , '<span class="layui-icon layim-tool-image" title="上传图片" layim-event="image">&#xe60d;<input type="file" name="file" accept="image/png,image/jpeg,image/jpeg,image/gif"></span>'
        , '{{# }; }}'
        ...
        // , '<span class="layui-icon layim-tool-audio" title="发送网络音频" layim-event="media" data-type="audio">&#xe6fc;<input type="file" name="file" accept="audio/*"></span>'
        // , '{{# }; }}'
        // , '{{# if(d.base && d.base.isVideo){ }}'
        // , '<span class="layui-icon layim-tool-video" title="发送网络视频" layim-event="media" data-type="video">&#xe6ed;<input type="file" name="file" accept="video/*"></span>'
        // , '{{# }; }}'
        , '{{# if(d.data && d.data.type==="group"){ }}'
        , '<span class="layui-icon layim-tool-files" title="群文件" layim-event="files">&#xe621;</span>'
        , '{{# }; }}'
        // , '<span class="layui-icon layim-tool-video" title="发起视频聊天" layim-event="voip" data-type="video">&#xe6ed;</span>'
    ...
    var elemAddTpl = ['<div class="layim-add-box">'
        , '<div class="layim-add-img"><img class="layui-circle" src="{{ d.data.avatar || d.base.defaultAvatar }}"><p>{{ d.data.name||"" }}</p></div>'
    ...
    //聊天内容列表模版
    var elemChatMain = ['{{# if(!d.system) {}}<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }} data-uid="{{d.fromid}}" data-senttime="{{d.message.sentTime}}" data-conversation={{d.message.conversationType}}>'
        , '<div class="layim-chat-user"><img src="{{ d.avatar || d.defaultAvatar }}"><cite>'
        , '{{# if(d.mine){ }}'
        ...
        , '{{# } }}'
        , '</cite></div>'
        , '<div class="layim-chat-text" data-content="{{ d.imgContent || d.content }}" data-filename="{{ d.file?d.file.name:""}}" data-filesize="{{ d.file?d.file.size:""}}" data-filetype="{{ d.file?d.file.type:""}}" data-fileurl="{{ (d.file?d.file.src:"")}}">{{ layui.data.content(d.content||"&nbsp") }}</div>'
        , '</li>'
        , '{{# } else {}}'
        , '<li class="layim-chat-system"><span>{{d.content}}</span></li>'
        , '{{# }}}'
    ].join('');

    var elemChatList = '<li class="layim-{{ d.data.type }}{{ d.data.id }} layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ (d.data.mine? d.data.targetAvatar : d.data.avatar) || d.base.defaultAvatar }}"><span>{{ (d.data.mine? d.data.targetName : d.data.name) ||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';
    ...
    layui.data.content = function (content) {
        ...
        content = (content || '').replace(/&(?!#?[a-zA-Z0-9]+;)/g, '&amp;')
            ...
            .replace(/@(\S+)(\s+?|$)/g, function (at) {
                var name = at.replace(/@|(\s+?|$)/g, '');
                var text = at.replace(/@(\S+)|/g, '');
                console.log(cache.mine.username);
                console.log(name);
                return '@<a '+(cache.mine.username==name?"class='mine'":"")+' href="javascript:;">'+name+'</a>'+text
            }) //转义@
            ...
            .replace(/img\([\s\S]+?\)\[([\s\S]+?)\]/g, function (img) {  //转义图片
                var url = (img.match(/img\(([\s\S]+?)\)\[/) || [])[1];
                var img = (img.match(/\)\[([\s\S]*?)\]/) || [])[1];
                if (img.indexOf("/9j/4AAQSkZJRgABAQAAAQABAAD/") >= 0) {//判断是否是二进制码
                    img = "data:image/png;base64," + img;
                }
                return '<img class="layui-layim-photos" src="' + img + '" data-url="'+url+'">';
            })
            ...
            .replace(/audio\[([\s\S]+?)\]/g, function (audio) {  //转义音频
            ...
            .replace(/voice\[([\s\S]+?)\]/g, function (voice) {  //转义音频
                return '<div class="layui-unselect layui-layim-audio" layim-event="playVoice" data-src="' + voice.replace(/(^voice\[)|(\]$)/g, '') + '"><i class="layui-icon">&#xe652;</i><p>音频消息</p></div>';
            })
            .replace(/video\[([\s\S]+?)\]/g, function (video) {  //转义音频
            ...
            .replace(/location\[([\s\S]+?)\]/g, function (location) {  //转义位置
                var params = location.replace(/(^location\[)|(\]$)/g, '').split(",");
                return '<a href="http://api.map.baidu.com/geocoder?address=' + params[3] + '&amp;output=html" target="_blank"><img class="layui-layim-photos layui-layim-location" data-latitude="' + params[1] + '" data-longitude="' + params[2] + '" data-poi="' + params[3] + '" src="data:image/png;base64,' + params[0] + '"></a>';
            })
            .replace(/businessCard\[([\s\S]+?)\]/g, function (businessCard) {  //转义名片
                var params = businessCard.replace(/(^businessCard\[)|(\]$)/g, '').split(",");
                return '<div class="layui-unselect layui-layim-business-card" layim-event="userInfo" data-uid="' + params[0] + '"><div class="layui-layim-business-card-body"><img class="layui-layim-business-card" src="'+params[2]+'"/>'+params[1]+'</div><div class="layui-layim-business-card-footer">个人名片</div> </div>';
            })
            ...
            .replace(/\n/g, '<br>') ;//转义换行
        content = RongIMLib.RongIMEmoji.emojiToHTML(content);//转义融云的表情
        return content;
    };
    ...
    var ajax = function (options, callback, tips) {
        options = options || {};
        return $.ajax({
            url: options.url
            , type: options.type || 'get'
    ...
    var cache = {message: {}, chat: []}, init = function (options) {
        ...
        mine = init.mine || {}
            ...
            //start 增加组织
            //设置默认图标
            cache.mine.avatar = cache.mine.avatar || cache.base.defaultAvatar;
            mainIndex = popim(laytpl(elemTpl).render(obj), obj);
            //end 增加组织
    ...
    //显示主面板
    var layimMain, popim = function (content, data) {
        return layer.open({
            type: 1
            , area: ['260px', '553px']
    ...
    var banRightMenu = function () {
        ...
        layimMain.find('.layim-list-group').on('contextmenu', 'li', function (e) {
            var othis = $(this);
            var html = '<ul data-id="' + othis[0].id + '" data-index="' + othis.data('index') + '" data-groupid="' + othis.data("id") + '"><li layim-event="menuGroup" data-type="view">查看群组</li>' +
                (othis.data("owner")==cache.mine.id?'<li layim-event="menuGroup" data-type="rename">修改群名称</li>':'') +
                '<li layim-event="menuGroup" data-type="quit">退出群组</li></ul>';
    ...
    var layimChat, layimMin, chatIndex, To = {}, popchat = function (data) {
        data = data || {};
        data.mine = data.fromid == cache.mine.id;
        ...
        //自定义消息的右键菜单
        $(document).on('contextmenu', '.layim-chat-text', function (e) {
            var thatChat = thisChat();
            var othis = $(this);
            var html = '<ul data-id="' + othis.parent().data("cid") + '" data-senttime="' + othis.parent().data("senttime") + '" data-conversation="' + othis.parent().data("conversation") + '" data-content="' + othis.data('content') + '" data-filename="' + othis.data('filename') + '" data-filesize="' + othis.data('filesize') + '" data-filetype="' + othis.data('filetype') + '" data-fileurl="' + othis.data('fileurl') + '">' +
                '<li layim-event="menuChatText" data-type="copy" data-clipboard-text="' + othis.data('content') + '">复制</li>' +
                '<li layim-event="menuChatText" data-type="clear">清空</li>';
                if(othis.parent().data("uid")==cache.mine.id){
                    //小于2分钟的消息可撤回
                    var sendTime = othis.parent().data("senttime");
                    var now =  Date.parse(new Date());
                    if(now - sendTime<=120000){
                        html +=  '<li layim-event="menuChatText" data-type="rollback">撤回</li>';
                    }
                    // html +=  '<li layim-event="menuChatText" data-type="remove">删除</li>';
                    html +=  '</ul>';
                }
            ...
            layer.tips(html, this, {
                ...
                , success: function (layero) {
                    ...
                    layero.off('mousedown', stopmp).on('mousedown', stopmp);
                    var client = new ZeroClipboard($("li[data-type=copy]"));
        ...
        var index = chatIndex = layer.open({
            ...
            success: function (layero) {
                ...
                typeof data.success === 'function' && data.result.success(layero);
                ...
                layero.on('dblclick', '.layui-layim-photos', function () {
                    var src = this.dataset.url || this.src;
        ...
        return popAdd.index = layer.open({
            ...
            , content: laytpl(elemAddTpl).render({
                data: {
                    ...
                    , avatar: data.avatar || cache.base.defaultAvatar
    ...
    var openUserInfo = function (uid) {
        var thatChat = thisChat();
        var userInfo = cache.base.userInfo;
        if(!uid){
            uid = thatChat.data.id;
        }
        var url = userInfo.url + uid;
        ajax({
            url: url
        }, function (data) {
            layer.open({
                ...
                content: '<p>姓名：' + data.user.nickname + '</p>'
                + '<p>性别：' + (data.profile.sex == 'm' ? '男' : '女') + '</p>'
    ...
    //打开群组信息面板
    var openGroupInfo = function (id) {
        var thatChat = thisChat();
        var groupInfo = cache.base.groupInfo;
        if(!id){
            id = thatChat.data.id;
        }
        var url = groupInfo.url + id;
        ajax({
            url: url
        }, function (data) {
            layer.open({
                ...
                anim: 2,
                shadeClose: true, //开启遮罩关闭
                content: '<p>群组名称：' + data.group.name + '</p>'
                + '<p>群组说明：' + data.group.desc + '</p>'
            });
        }, 'INIT');
    };

    var quitFromGroup = function (id, uid) {
        var groupQuit = cache.base.groupQuit;
        var url = groupQuit.url;
        ajax({
            type : groupQuit.type
            , url: url
            , data : {
            ...
            if(data.result.success){
                toastr.success("退出群组成功");
            }
        }, 'INIT');
    };

    var kickFromGroup = function (id, uid) {
        var groupKick = cache.base.groupKick;
        var url = groupKick.url;
        ajax({
            type : groupKick.type
            , url: url
            , data : {
    ...
    var sendMessage = function () {
        ...
        var maxLength = cache.base.maxLength || 100000;
        data.content = thatChat.textarea.val();
        data.file = fileData;
        data.imgContent = thatChat.textarea.val();
        if (data.content.replace(/\s/g, '') !== '') {

            //图片因为有缩略图信息，不限制长度
            if (data.content.indexOf("img[") < 0 && data.content.length > maxLength) {
    ...
    var messageNew = {}, getMessage = function (data) {
        data = data || {};
        data.defaultAvatar = cache.base.defaultAvatar;
            ...
            // if (!data.system) {
                if (cache.base.notice) {
                    notice({
                        ...
                        , avatar: group.avatar || data.avatar || cache.base.defaultAvatar
                    });
                }
                var name = data.content.replace(/@|(\s+?|$)/g, '');
                if(!atFlag){
                    atFlag = name && name==cache.mine.username;
                }

                return setChatMin({
                    name: atFlag?"[有人@你]":(data.fromid == cache.mine.id ? '收到同步消息' : '收到新消息')
                    ,
                    avatar: (data.fromid == cache.mine.id ? data.targetAvatar : group.avatar || data.avatar) || cache.base.defaultAvatar
                    ,
                    anim: 6
                });
            }
    ...
    var pushChatlog = function (message) {
        ...
        if (thisChatlog) {
            ...
            if (!(nosame || message.id == cache.mine.id)) {
    ...
    var addList = function (data) {
        ...
        if (cache[data.type]) {
            if (data.type === 'friend') {
                ...
                if (has) return layer.msg('您已是 [' + (data.name || '') + '] 的群成员', {anim: 6});
    ...
    var anim = 'layui-anim-upbit', events = {
        ...
        , search: function (othis) {
            ...
            var input = search.find('input'), find = function (e) {
                ...
                if (val === '') {
                    ...
                    //搜索组织 start
                    console.log(cache.org);
                    var org = cache.org || [];
                    var tempData = [];
                    iteration(org, data, val);
                    cache.orgData = data;
                    //end
                    ...
                    if (data.length > 0) {
                        for (var l = 0; l < data.length; l++) {
                            html += '<li layim-event="chat" data-type="' + data[l].type + '" data-index="' + data[l].index + '" data-list="' + data[l].list + '"><img src="' + data[l].avatar + '"><span>' + (data[l].username || data[l].name || '佚名') + '</span><p>' + (data[l].remark || data[l].sign || '') + '</p></li>';
                        }
                    } else {
        ...
        , chat: function (othis) {
            ...
            var list = othis.data('list') || othis.index(), data = {};
            if (type === 'friend') {
                console.log(list);
                data = cache[type][index].list[othis.index()];
            } else if (type === 'group') {
                data = cache[type][list];
            } else if (type === 'history') {
                data = (local.history || {})[index] || {};
            } else if (type === 'org') { //增加组织机构处理
                data = cache[type][0].us[list];
            }
            data.name = data.name || data.username || data.groupname;
            if (type == 'friend' || type == 'org') {
                data.type = "";
            } else if (type !== 'history') {
                data.type = type;
            }
        ...
        , face: function (othis, e) {
            ...
            for (var i in emojis) {
                content += '<li title="' + emojis[i].children[0].getAttribute("name") + '">'+emojis[i].innerHTML+'</li>';
            }
        ...
        , userInfo: function (othis) {
            openUserInfo(othis.data("uid"));
        }
        , groupInfo: function (othis) {
            openGroupInfo();
        }
        //图片或一般文件
                ...
                , success: function (res) {
                    if (res.result.success) {
                        ...
                        if (type === 'images') {
                            ...
                            console.log(othis.find('input')[0].files);
                            ImageHelper.getThumbnail(othis.find('input')[0].files[0], 60000, function (obj, data) {
                                focusInsert(thatChat.textarea[0], "img("+res.data.src+")[" + data + "]");
                                sendMessage();
                            });
                        } else if (type === 'file') {
                    ...
                    // othis.find('input')[0].value = '';
                }
            });
        }
        //群文件
        , files: function (othis) {
            var thatChat = thisChat();
            groupId = thatChat.data.id;
            var groupFiles = cache.base.groupFiles;
            ajax({
                url: groupFiles.url
                , data : {
                    groupId : thatChat.data.id
                ...
                if (res.result.success) {
                    ...
                    for(var i in messages){
                        ...
                        var file = eval("("+message.content+")");
                        content += "<tr>"
                                    +"<td>"+file.name+"</td>"
                                    +"<td>"+(new Date(message.timestamp)).format("yyyy年MM月dd日 hh:mm")+"</td>"
                                    +"<td>"+sizeConvert(file.size)+"</td>"
                                    +"<td>"+file.user.name+"</td>"
                                    +"<td><a class='btn btn-info btn-sm' href='"+file.fileUrl+"'>下载</a></td>"
                                    +"</tr>";
                    }
                    content += "</tbody></table>";
                    content += generatePage(res.page);
                    layer.open({
                        id : "files",
                        type: 0,
                        title: thatChat.data.name,
                        btn: [],
                        area: ['800px', 'auto'],
                        anim: 2,
                        shadeClose: true, //开启遮罩关闭
                    ...
                    layer.msg(res.msg || '打开群文件失败');
                }
            });
        }
        //音频和视频
        , media: function (othis) {
            var type = othis.data('type'), api = {
                audio: 'uploadAudio'
                , video: 'uploadVideo'
            }, thatChat = thisChat(), upload = cache.base[api[type]] || {};
            layui.upload({
                ...
                , success: function (res) {
                    if (res.result.success) {
                        res.data = res.data || {};
                        fileData = res.data;
                        focusInsert(thatChat.textarea[0], type + '[' + (res.data.src || '') + ']');
                        sendMessage();
                    } else {
                        layer.msg(res.msg || '上传失败');
                    }
        ...
        , voip: function (othis) {
            ...
            var param = {
                targetId: thatChat.data.id,
                uid: cache.mine.id
            }

            layui.each(call.startVoIP, function (index, item) {
                item && item(param);
            });
        }
        //扩展工具栏
        ...
        , playVoice: function (othis) {
            var audioFile = othis.data("src");
            var duration = audioFile.length/1024;
            othis.find('i').html('&#xe688;').addClass("layui-anim").addClass("layui-anim-rotate").addClass("layui-anim-loop");
        ...
        , playVideo: function (othis) {
            ...
            events.playVideo.index = layer.open({
                ...
                shade: false
                ,
                content: '<div style="background-color: #000; height: 100%;"><video style="position: absolute; width: 100%; height: 100%;" src="' + videoData + '" poster="" loop="loop" autoplay="autoplay"></video></div>'
            });
        }
        ...
        , menuChatText: function (othis, e) {
            ...
            if (type === 'copy') {
                fileData = {
                    name: parent.data('filename'),
                    size: parent.data('filesize'),
                    type: parent.data('filetype'),
                    src: parent.data('fileurl')
                };
                console.log(thatChat);
                thatChat.textarea.val(content);

            } else if (type === 'clear') {
                thatChat.elem.find(".layim-chat-main").children("ul").html("");
            } else if (type === 'rollback') {
                layui.each(call.rollback, function (index, item) {
                    item && item({id:parent.data("id"),targetId:thatChat.data.id, nickname:cache.mine.username, conversationType:parent.data("conversation"),sentTime:parent.data("senttime")});
                });
            }
        ...
        , menuGroup: function (othis, e) {
            ...
            switch (type) {
                case "view":
                    ...
                    break;
                case "rename":
                    layer.prompt({title: '请输入群名称', formType: 0}, function (name, index) {
                        layer.close(index);
                        renameGroup(groupId, name, cache.mine.id, function () {

                        });
                    });
                    break;
                case "quit":
                    quitFromGroup(groupId, cache.mine.id);
                    break;
            }
...
//遍历组织架构，返回搜索结果
function iteration(org, data, val) {
    console.log(org.length);
    for (var i = 0; i < org.length; i++) {
        for (var j in org[i].users) {
            if (org[i].users[j].username.indexOf(val) !== -1) {
                var friend = org[i].users[j];
                friend.type = 'private';
                friend.index = data.length;
                friend.list = "";
                friend.username = org[i].users[j].username;
                friend.id = org[i].users[j].id;
                friend.avatar = org[i].users[j].avatar;
                friend.sign = org[i].users[j].sign;
                data.push(friend);
            }
        }

        if (org[i].children != null && org[i].children.length > 0) {
            iteration(org[i].children, data, val);
        }
    }
}